...
function buildBudgetPrompt(): string {
...
You have at most 6 queries. Budget them wisely. After each query you will be reminded of remaining budget.

Format: QUERY: x0,x1,x2 or ANSWER: rule_id (EXACT ID)`;
}

async function runBudgetSingle(task: Task, config: RunConfig): Promise<RunResult> {
  ...
  const messages: { role: string; content: string }[] = [
    { role: 'system', content: buildBudgetPrompt() },
    { role: 'user', content: `Observations:\n${obsStr}\n\nVS: ${task.versionSpaceRuleIds.length} rules. Budget: 6 queries. First query?` },
  ];
  ...
  for (let step = 0; step < 14; step++) {
    ...
    for (const q of p.queries) { if (!env.isDone()) { const qr = env.query(q); queryResults.push(qr); const rem = 6 - env.queriesMade; messages.push({ role: 'user', content: `(${q[0]},${q[1]},${q[2]}) -> ${qr.result}\nVS: ${qr.version_space_size}${qr.is_duplicate ? ' [DUP]' : ''}\n\nBUDGET REMINDER: ${rem} queries remaining.${rem <= 2 ? ' Use wisely or ANSWER now!' : ''}` }); } }
...
async function runBudgetBatch(tasks: Task[], config: RunConfig, outputPath: string): Promise<void> {
  const dir = path.dirname(outputPath); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(outputPath, '');
  for (let i = 0; i < tasks.length; i++) {
    console.log(`[${i + 1}/${tasks.length}] Budget ${tasks[i].taskId}...`);
    try { const r = await runBudgetSingle(tasks[i], config); fs.appendFileSync(outputPath, JSON.stringify(r) + '\n'); console.log(`  -> ${r.predictedRuleId}, correct: ${r.correct}, queries: ${r.queriesMade}`); }
  ...
  console.log(`Budget batch complete. ${outputPath}`);
}

if (process.argv[1] && process.argv[1].endsWith('runActiveBudgetReminder.ts')) {
  const args = process.argv.slice(2);
  let tasksPath = 'results/tasks_seed42.json', model = 'deepseek-chat', output = 'results/results_active_budget_reminder.jsonl', seed = 42;
  ...
  await runBudgetBatch(tasksData, { ...DEFAULT_CONFIG, model, seed }, output);
}
