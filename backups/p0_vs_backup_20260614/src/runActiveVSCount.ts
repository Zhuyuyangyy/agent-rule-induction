...
function buildVSCountPrompt(): string {
...
You have at most 6 queries. After each query, you will be told exactly how many candidate rules remain. Use this to decide whether to query more or answer.

Format: QUERY: x0,x1,x2 or ANSWER: rule_id (EXACT ID)`;
}

async function runVSCountSingle(task: Task, config: RunConfig): Promise<RunResult> {
  ...
  const messages: { role: string; content: string }[] = [
    { role: 'system', content: buildVSCountPrompt() },
    { role: 'user', content: `Observations:\n${obsStr}\n\nVS size: ${task.versionSpaceRuleIds.length} rules. Queries left: 6. First query?` },
  ];
  ...
  for (let step = 0; step < 14; step++) {
    ...
    for (const q of p.queries) { if (!env.isDone()) { const qr = env.query(q); queryResults.push(qr); messages.push({ role: 'user', content: `(${q[0]},${q[1]},${q[2]}) -> ${qr.result}\n\nVS: ${qr.version_space_size} rules remain${qr.is_duplicate ? ' [DUP]' : ''}\nQueries left: ${6 - env.queriesMade}${qr.version_space_size === 1 ? '\nOnly 1 rule left - answer now!' : ''}` }); } }
...
async function runVSCountBatch(tasks: Task[], config: RunConfig, outputPath: string): Promise<void> {
  const dir = path.dirname(outputPath); if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  fs.writeFileSync(outputPath, '');
  for (let i = 0; i < tasks.length; i++) {
    console.log(`[${i + 1}/${tasks.length}] VSCount ${tasks[i].taskId}...`);
    try { const r = await runVSCountSingle(tasks[i], config); fs.appendFileSync(outputPath, JSON.stringify(r) + '\n'); console.log(`  -> ${r.predictedRuleId}, correct: ${r.correct}, queries: ${r.queriesMade}`); }
  ...
  console.log(`VSCount batch complete. ${outputPath}`);
}

if (process.argv[1] && process.argv[1].endsWith('runActiveVSCount.ts')) {
  const args = process.argv.slice(2);
  let tasksPath = 'results/tasks_seed42.json', model = 'deepseek-chat', output = 'results/results_active_vscount.jsonl', seed = 42;
  ...
  await runVSCountBatch(tasksData, { ...DEFAULT_CONFIG, model, seed }, output);
}
