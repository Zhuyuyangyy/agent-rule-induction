...
Format: QUERY: x0,x1,x2

After all queries, provide structured final answer:
TOP_3_HYPOTHESES:
1. rule_id - confidence(0-1) - key_evidence
2. rule_id - confidence(0-1) - key_evidence
3. rule_id - confidence(0-1) - key_evidence
...
async function runBatch(tasks: Task[], config: RunConfig, outputPath: string): Promise<void> {
  ...
  for (let i = 0; i < tasks.length; i++) {
    console.log(`[${i + 1}/${tasks.length}] FinalOnlyScaffold ${tasks[i].taskId}...`);
  ...
  console.log(`FinalOnlyScaffold batch complete. ${outputPath}`);
}

if (process.argv[1] && process.argv[1].endsWith('runFinalOnlyScaffold.ts')) {
  const args = process.argv.slice(2);
  let tasksPath = 'results/tasks_seed42.json', model = 'deepseek-chat', output = 'results/results_final_only_scaffold.jsonl', seed = 42;
