...
You have at most 6 queries. For each query, provide structured reasoning:

QUERY_STEP:
- top_2_candidates: The 2 most likely rules and why
- why_discriminative: Why this query distinguishes between them
- QUERY: x0,x1,x2

After all queries, provide a simple final answer:
ANSWER: rule_id (EXACT ID)`;
}
...
async function runSingle(task: Task, config: RunConfig): Promise<RunResult> {
  ...
  const messages: { role: string; content: string }[] = [
    ...
    { role: 'user', content: `Observations:\n${obsStr}\n\nVS: ${task.versionSpaceRuleIds.length} rules. First QUERY_STEP?` },
  ];
  ...
  for (let step = 0; step < 14; step++) {
    ...
    if (p.queries.length === 0 && !p.answer) { messages.push({ role: 'user', content: env.isDone() ? 'No more queries. You MUST output ANSWER: rule_id' : `QUERY: x0,x1,x2 or ANSWER: rule_id. Left: ${6 - env.queriesMade}` }); }
...
async function runBatch(tasks: Task[], config: RunConfig, outputPath: string): Promise<void> {
  ...
  for (let i = 0; i < tasks.length; i++) {
    console.log(`[${i + 1}/${tasks.length}] QueryOnlyScaffold ${tasks[i].taskId}...`);
  ...
  console.log(`QueryOnlyScaffold batch complete. ${outputPath}`);
}

if (process.argv[1] && process.argv[1].endsWith('runQueryOnlyScaffold.ts')) {
  const args = process.argv.slice(2);
  let tasksPath = 'results/tasks_seed42.json', model = 'deepseek-chat', output = 'results/results_query_only_scaffold.jsonl', seed = 42;
